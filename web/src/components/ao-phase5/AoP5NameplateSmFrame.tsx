...
/** Name_SM 自然サイズ（外寸算出用。描画は Plate_A / label） */
...
/**
 * 名前ラベル = Plate_A（`label` プリセット）。外寸算出は従来どおり。
 */
export function AoP5NameplateSmFrame({
  ...
  const geomScale = tight ? scale * 0.55 : scale;
  const cnr = Math.max(6, Math.round(NAME_CNR_NAT * geomScale));
  const tbH = Math.max(2, Math.round(NAME_BTM_H_NAT * geomScale));
  const riteW = Math.max(3, Math.round(NAME_RITE_W_NAT * geomScale));
  ...
  const innerH = Math.max(fontSize + 4, Math.round(NAME_BTM_H_NAT * innerHDepthScale * 1.5));
  ...
  return (
    <AoTemplateFrame
      ...
      contentStyle={{
        ...
        overflow: lockOuterWidth ? "hidden" : "visible",
        color: "#2a1406",
        fontWeight: 800,
