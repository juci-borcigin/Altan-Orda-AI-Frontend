...
function SummaryBlock({ title, a }: { title: string; a?: Agg }) {
  if (!a) return null;
  return (
    <div className="border-b pb-1.5 mb-1.5" style={{ borderColor: `${AO_INK}22` }}>
      <h4 className="mb-0.5 text-[10px] font-semibold text-[#3D1C08]">{title}</h4>
      <div className="grid gap-0.5 font-mono text-[10px] text-[#3D1C08]/80">
        <div>
    ...
    >
      ...
      <div className={bodyClass} style={bodyStyle}>
        {loading ? (
          ...
          <div className="space-y-0.5 pb-3 font-serif">
            <div className="mb-1 text-[10px] text-[#3D1C08]/70">
              応答サンプル数（DB 行）:{" "}
            ...
            <h4 className="mb-0.5 mt-2 text-[10px] font-semibold text-[#3D1C08]">
              ベンダー別（model_id の接頭辞・直近 {data?.windowDays ?? 30} 日）
            </h4>
            <div className="space-y-1 font-mono text-[10px] text-[#3D1C08]/70">
              {vendorEntries.length === 0 ? (
                ...
                vendorEntries.map(([k, v]) => (
                  <div key={k} className="flex flex-wrap justify-between gap-x-2 border-b border-[#3D1C08]/20 py-0.5">
