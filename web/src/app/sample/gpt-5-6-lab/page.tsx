...
export default function Gpt56LabPage() {
  ...
  useEffect(() => {
    void fetch("/api/sample/gpt-5-6-lab", { cache: "no-store" })
      .then(async (response) => {
  ...
  return (
    ...
    >
      ...
      <h1 style={{ margin: "0.35rem 0", fontSize: "1.7rem" }}>GPT-5.6 講座生成テスト</h1>
      <p style={{ margin: "0 0 0.75rem" }}>
        <Link href="/sample">Sampleトップへ</Link>
        {" · "}
        <Link href="/sample/session1-visual-lab">第1回画像比較へ</Link>
        {" · "}
        <Link href="/sample/text-lab">旧モデル文章比較へ</Link>
        {" · "}
        <Link href="/sample/image-lab">画像比較へ</Link>
        {" · "}
