...
export default function TextLabPage() {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/sample/text-lab", { cache: "no-store" });
  ...
  return (
    ...
    >
      ...
      <h1 style={{ margin: "0.35rem 0 0.4rem", fontSize: "1.7rem" }}>本文・講座構成比較</h1>
      ...
      <p style={{ margin: "0 0 1rem", fontSize: "0.78rem" }}>
        <Link href="/sample/image-lab">画像比較ラボへ</Link>
        {" · "}
        <Link href="/sample/gpt-5-6-lab">GPT-5.6講座生成テストへ</Link>
        {" · "}
        <button
          type="button"
