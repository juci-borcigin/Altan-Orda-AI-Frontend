...
/**
 * Supabase `ao_prompts` を読み、`buildAoSystemPrompt` へ渡す上書きマップにする。
 * テーブル不存在・空・エラー時は `{}`（コード既定のみ）。
 */
export async function loadAoPromptOverrides(
  supa: SupabaseClient,
): Promise<Partial<Record<AoPromptSectionKey, string>>> {
  const { data, error } = await supa.from("ao_prompts").select("section_key, body");
  if (error) {
    console.error("[ao-prompt] load ao_prompts:", error.message);
    return {};
  }
