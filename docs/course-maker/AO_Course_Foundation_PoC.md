# 講習メーカー — 構成フェーズ拡張 PoC 設計（2026-07-20）

ステータス: **型・見積もり骨格実装済み / 課金ランナー未接続**

前提（確定）:

- ドラフト粒度 = **講座全体**（回ごと詳細ドラフトではない）
- 監査 = **要点＋上限付き**（全主張の網羅 FC はしない）
- 講習用 RAG = 当面なし（確度は監査ゲートで担保）
...
type ContentDraft = {
  ...
  audience: string;
  /** 講座全体の教える中身。見出し階層つき Markdown 想定（回割はまだしない） */
...
| 予算キャップ（目安） | **$0.50 / 講座** | 超過で停止しレポート途中保存 |
...
2. 成果物は `public/sample/course-foundation-poc/manifest.json`
3. UI: `/sample/course-foundation-poc`
...
| Sample API ガード | PoC 生成 API も同じ `guardSampleApiMutation` を使う |
